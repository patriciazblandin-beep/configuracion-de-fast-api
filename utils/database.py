...
async def get_db_connection():
    try: 
        logger.info(f"intentndo conectar a la base de datos...")
        conn = pyodbc.connect(connection_string, timeout=10)
        logger.info("Conexion exitosa a la base de datos")
        return conn
    except pyodbc.Error as e:
        logger.error (f"Error de la conexion a la base de datos: {str(e)}")
        raise Exception (f" Error de connecion a la base de datos:  {str(e)}")
    except Exception as e:
        logger.error(f" Error inesperado durante la connecion:  {str(e)}")
        raise
